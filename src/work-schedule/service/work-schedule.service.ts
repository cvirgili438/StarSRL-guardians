...
export class WorkScheduleService {
  ...
  public async createSchedule(
    ...
    try {
      const workSchedule: WorkScheduleEntity =
        await this.workScheduleRepository.create(body);
  ...
  public async findAllSchedule(): Promise<WorkScheduleEntity[]> {
    try {
      const schedules: WorkScheduleEntity[] =
  ...
  public async findSchedulesOfUser(id: string): Promise<WorkScheduleEntity[]> {
    try {
      ...
      if (schedules.length === 0) {
        throw new ErrorManager({
          type: 'BAD_REQUEST',
          message: `Cannot find the users id =${id} Schedules `,
        });
      } else return schedules;
    } catch (error) {
      throw ErrorManager.createSignatureError(error.message);
  ...
  public async userSchedulePlace(
    body: UserSchedulePlaceDTO,
  ): Promise<WorkScheduleEntity> {
    ...
    try {
      ...
      if (verify) {
        throw new ErrorManager({
          type: 'AMBIGUOUS',
          message:
            'The Schedule already exist,please change the exist schedule id ' +
            verify.id,
        });
      }
      let verify2: WorkScheduleEntity[] = await this.workScheduleRepository
        .createQueryBuilder('schedule')
        .where(
          'schedule.workPlace = :workPlace AND schedule.month = :month AND schedule.dayOfWeek = :dayOfWeek AND schedule.year = :year',
          {
            workPlace: workPlace,
            month: month,
            dayOfWeek: dayOfWeek,
  ...
  public async putWorking(
    id: string,
    body: StartOrEndingWorkDTO,
  ): Promise<WorkScheduleEntity> {
    try {
  ...
  public async getFilterSchedule(
    body: GetFilterScheduleDTO,
    id: string,
  ): Promise<WorkScheduleEntity[]> {
    try {
      const getSchedules: WorkScheduleEntity[] =
        await this.workScheduleRepository
          .createQueryBuilder('schedules')
          .where(
            'user.id = :id AND schedules.month = :month AND schedules.year = :year',
            {
              id,
              month: body.month,
              year: body.year,
      ...
      if (getSchedules.length > 0) return getSchedules;
        ...
        throw new ErrorManager({
          type: 'BAD_REQUEST',
          message: 'Dont find any schedules for this user',
        });
      }
    } catch (error) {
        throw ErrorManager.createSignatureError(error.message)
