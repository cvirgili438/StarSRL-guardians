...
export class WorkPlacesService {
  ...
  public async createWorkPlace(body: workPlacesDTO): Promise<WorkPlacesEntity> {
    try {
      const workPlace: WorkPlacesEntity = await this.workPlacesRepository.save(
        body,
      );
      if (!workPlace) {
  ...
  public async deleteWorkPlace(id: string): Promise<DeleteResult> {
    try {
      const workPlace: DeleteResult = await this.workPlacesRepository.delete(
        id,
      );
      if (workPlace.affected === 0) {
        throw new ErrorManager({
  ...
  public async getByName(body: any): Promise<WorkPlacesEntity[]> {
    try {
      let places = await this.workPlacesRepository
